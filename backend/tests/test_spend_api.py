...
def test_spend_settles_with_fee_split_to_fee_wallet(components: Components) -> None:
    ...
    assert len(components.receipts.recorded) == 1
    assert components.receipts.recorded[0]["fee_tx_hash"] == "0xfeepaid"


def test_spend_fee_transfer_failure_still_settles_payment(
    components: Components,
) -> None:
    mandate = _create_mandate(components.store)

    def fail_fee(**kwargs: object) -> str:
        raise FeeTransferError("The fee transfer call failed.")

    components.fees.collect_fee = fail_fee  # type: ignore[method-assign]
