...
def create_app(
    ...
    def list_mandates(identity: identity_dependency) -> JSONResponse:
        ...
        return JSONResponse(content={"mandates": [_mandate_to_json(m) for m in mandates]})
    ...
    def get_mandate_status(
        ...
        if active_status is None:
            raise StarletteHTTPException(status_code=503)
        try:
            status = active_status.status(user_id=identity.subject, mandate_id=mandate_id)
        except NotFoundError:
            raise StarletteHTTPException(status_code=404) from None
        return JSONResponse(content=_status_to_json(status))
    ...
    def list_mandate_receipts(
        ...
        try:
            ...
            raise StarletteHTTPException(status_code=404) from None
        agent_identity = status.mandate.agent_identity
        receipts = active_receipts.list_receipts(user_id=agent_identity)
        return JSONResponse(content={"receipts": [_receipt_to_json(r) for r in receipts]})
...
def _mandate_to_json(mandate: Mandate) -> dict[str, object]:
    """Render one mandate as a safe JSON document for the list endpoint."""
    return {
        "id": str(mandate.id),
        "user_id": mandate.user_id,
        "agent_identity": mandate.agent_identity,
...
def _status_to_json(status: MandateStatus) -> dict[str, object]:
    """Render one mandate status document for the dashboard (ticket 08)."""
...
def _intent_to_json(intent: Intent) -> dict[str, object]:
    """Render one intent for the status document."""
    return {
        "id": str(intent.id),
        "purpose_hash": intent.purpose_hash,
        "service_url": intent.service_url,
...
def _breaker_to_json(state: BreakerState) -> dict[str, object]:
    """Render one breaker state for the status document."""
    return {
...
def _receipt_to_json(receipt: ArcReceipt) -> dict[str, object]:
    """Render one on-Arc receipt for the receipts endpoint."""
    return {
