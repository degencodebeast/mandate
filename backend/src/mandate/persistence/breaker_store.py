...
- consume_trial: atomically consume the single HALF_OPEN trial.
...
class PostgresBreakerStateStore:
        ...
        with psycopg.connect(self._database_url, row_factory=dict_row) as connection:
            ...
            if row is None:
                connection.execute(
                    ...
                    ) VALUES (%s, %s, 0, 'closed', NULL, false)
                    """,
    ...
    def record_failure(
        ...
        is always refreshed so the OPEN cooldown restarts from this moment.
        """
        with psycopg.connect(self._database_url, row_factory=dict_row) as connection:
            row = connection.execute(
                """
                ...
                ON CONFLICT (service_url) DO UPDATE SET
                    ...
                    END
                RETURNING service_url, failure_count, state, last_failure_at, trial_allowed
                """,
    ...
    def open_to_half_open(self, *, service_url: str) -> BreakerState | None:
        """Move an OPEN breaker to HALF_OPEN and allow the trial."""
        with psycopg.connect(self._database_url, row_factory=dict_row) as connection:
            row = connection.execute(
                """
